...
class EngineInfo
{
	...
	// functions :
		...
		bool check_for_engine_module( const char* pName );
        
#ifdef _WIN32

        // Set info using the PE header found by module name.
        // Returns 0 on success, error code on failure.
        int  nthdr_module_name( void );

		int vac_pe_approx( enginefuncs_t* pFuncs );
    ...
    // functions :
		...
		int initialise( enginefuncs_t* pFuncs  = NULL );
        
        // Test if pMem is within bounds of the code segment.
		bool is_valid_code_pointer( void* pMem );

        // Overloaded versions of above test to keep the ugly pointer
        // conversion stuff in here.
        bool is_valid_code_pointer(      const char* (*fp) (edict_t*) );
        bool is_valid_code_pointer( sequenceEntry_s* (*fp) (const char*, const char*) );
        bool is_valid_code_pointer( sentenceEntry_s* (*fp) (const char*, int, int*) );
        bool is_valid_code_pointer(              int (*fp) (char*) );
        bool is_valid_code_pointer(     unsigned int (*fp) (const char*) );
        bool is_valid_code_pointer(              int (*fp) (void) );
        bool is_valid_code_pointer(              int (*fp) (const char*) );
        bool is_valid_code_pointer(             void (*fp) (int) );
        bool is_valid_code_pointer(              int (*fp) (int) );
        bool is_valid_code_pointer(             void (*fp) (int*, int) );
        bool is_valid_code_pointer(             void (*fp) (void) );
        bool is_valid_code_pointer(             void (*fp) (const edict_t*, const char*) );
        bool is_valid_code_pointer(             void (*fp) (const edict_t*, const char*, int) );
        bool is_valid_code_pointer(              int (*fp) (const char *, char**) );
