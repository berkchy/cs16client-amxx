...
#ifdef __ANDROID__
	{
		// Android loads game libraries from the app's native library
		// directory, which is where this metamod library itself lives.
		// Plain dlopen() doesn't know about that directory, so resolve the
		// plugin path relative to our own location.
		char tmp[MAX_PATH];
		Q_strlcpy(tmp, m_pathname);
		const char *base = Q_strrchr(tmp, '/');
		base = base ? base + 1 : tmp;
		const char *selfpath = g_metamod_module.getpath();
		if (selfpath) {
			char selfdir[MAX_PATH];
			Q_snprintf(selfdir, sizeof selfdir, "%s", selfpath);
			char *slash = Q_strrchr(selfdir, '/');
			if (slash)
				*slash = '\0';
			Q_snprintf(m_pathname, sizeof m_pathname, "%s/%s", selfdir, base);
		}
	}
#endif
