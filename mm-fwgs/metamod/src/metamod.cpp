...
void metamod_startup()
{
	...
	if (!is_file_exists_in_gamedir(configFile)) {
		...
		if (!is_file_exists_in_gamedir(configFile)) {
#ifdef __ANDROID__
			// The metamod directory on Android is the APK native library
			// dir, where no config.ini can live. Fall back to the regular
			// addons/metamod directory of the game dir.
			char cfgrel[MAX_PATH];
			char cfgabs[MAX_PATH];
			Q_snprintf(cfgrel, sizeof cfgrel, "addons/metamod/%s", CONFIG_INI);
			if (is_file_exists_in_gamedir(cfgrel)) {
				full_gamedir_path(cfgrel, cfgabs);
				Q_strlcpy(configFile, cfgabs);
			}
			else {
	...
	if (!is_file_exists_in_gamedir(pluginFile)) {
		...
		if (!is_file_exists_in_gamedir(pluginFile)) {
#ifdef __ANDROID__
			// Fall back to the regular addons/metamod directory of the game dir.
			char plrel[MAX_PATH];
			char plabs[MAX_PATH];
			Q_snprintf(plrel, sizeof plrel, "addons/metamod/%s", PLUGINS_INI);
			if (is_file_exists_in_gamedir(plrel)) {
				full_gamedir_path(plrel, plabs);
				if (!is_abs_path(plabs)) {
					Q_snprintf(plabs, sizeof plabs, "%s/%s", g_GameDLL.gamedir, plrel);
				}
				Q_strlcpy(pluginFile, plabs);
			}
			else {
...
#ifdef __ANDROID__
	{
		FILE *dbg = fopen("/storage/emulated/0/xash/mm_dbg.txt", "a");
		if (dbg) {
			fprintf(dbg, "pluginFile=%s exists=%d\n", pluginFile, is_file_exists_in_gamedir(pluginFile));
			fclose(dbg);
		}
	}
#endif
...
bool meta_load_gamedll()
{
...

	{
		FILE *dbg = fopen("/storage/emulated/0/xash/mm_dbg.txt", "a");
		if (dbg) {
			fprintf(dbg, "meta_load_gamedll: gamedir=%s cfg_gamedll=%s selfpath=%s\n",
				g_GameDLL.gamedir, g_config->m_gamedll ? g_config->m_gamedll : "(null)",
				g_metamod_module.getpath() ? g_metamod_module.getpath() : "(null)");
			fprintf(dbg, "  pathname=%s\n", g_GameDLL.pathname);
			fclose(dbg);
		}
	}
