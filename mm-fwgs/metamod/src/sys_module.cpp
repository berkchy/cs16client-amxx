...
module_handle_t CSysModule::load(void *addr)
{
	...
	Q_strlcpy(m_selfpath, dlinfo.dli_fname);

	m_handle = dlopen(dlinfo.dli_fname, RTLD_NOW | RTLD_NOLOAD);
	return m_handle;
}

const char *CSysModule::getpath() const
{
	return m_selfpath[0] ? m_selfpath : nullptr;
}
...
#ifdef __ANDROID__
		// RTLD_DEEPBIND can prevent the game lib from resolving globals
		// exported by already-loaded engine libraries. Retry without it.
		FILE *dbg = fopen("/storage/emulated/0/xash/mm_dbg.txt", "a");
		if (m_handle) {
			if (dbg) { fprintf(dbg, "dlopen(DEEPBIND) OK  %s\n", filepath); fclose(dbg); }
		} else {
			if (dbg) { fprintf(dbg, "dlopen(DEEPBIND) FAIL %s: %s\n", filepath, dlerror()); fclose(dbg); }
			m_handle = dlopen(filepath, RTLD_NOW | RTLD_LOCAL);
			dbg = fopen("/storage/emulated/0/xash/mm_dbg.txt", "a");
			if (m_handle) {
				if (dbg) { fprintf(dbg, "dlopen(plain) OK  %s\n", filepath); fclose(dbg); }
			} else {
				if (dbg) { fprintf(dbg, "dlopen(plain) FAIL %s: %s\n", filepath, dlerror()); fclose(dbg); }
				if (dbg) {
					char sdir[4096];
					Q_strlcpy(sdir, filepath);
					char *sl = Q_strrchr(sdir, '/');
					if (sl) *sl = '\0';
					DIR *d = opendir(sdir);
					if (!d) {
						fprintf(dbg, "  opendir %s: %s\n", sdir, strerror(errno));
					} else {
						struct dirent *de;
