...
static inline cell ToCell(int v) { return static_cast<cell>(v); }
static inline cell ToCell(unsigned int v) { return static_cast<cell>(v); }
static inline cell ToCell(long v) { return static_cast<cell>(v); }
static inline cell ToCell(unsigned long v) { return static_cast<cell>(v); }
static inline cell ToCell(long long v) { return static_cast<cell>(v); }
static inline cell ToCell(unsigned long long v) { return static_cast<cell>(v); }
static inline cell ToCell(short v) { return static_cast<cell>(v); }
static inline cell ToCell(unsigned short v) { return static_cast<cell>(v); }
static inline cell ToCell(bool v) { return static_cast<cell>(v ? 1 : 0); }
static inline cell ToCell(double v) { return static_cast<cell>(static_cast<intptr_t>(v)); }
template <typename T> static inline cell ToCell(T *p) { return static_cast<cell>(reinterpret_cast<intptr_t>(p)); }
...
{
	FM_ENG_HANDLE(FM_ClientUserInfoChanged, (Engine[FM_ClientUserInfoChanged].at(i), ToCell(ENTINDEX(e)), ToCell(infobuffer)));
	RETURN_META(mswi(lastFmRes));
}

void ClientUserInfoChanged_post(edict_t *e, char *infobuffer)
{
	FM_ENG_HANDLE_POST(FM_ClientUserInfoChanged, (EnginePost[FM_ClientUserInfoChanged].at(i), ToCell(ENTINDEX(e)), ToCell(infobuffer))); 
	RETURN_META(MRES_IGNORED);
}

void SetModel(edict_t *e, const char *m)
{
	FM_ENG_HANDLE(FM_SetModel, (Engine[FM_SetModel].at(i), ToCell(ENTINDEX(e)), m));
	RETURN_META(mswi(lastFmRes));
}
void SetModel_post(edict_t *e, const char *m)
{
	FM_ENG_HANDLE_POST(FM_SetModel, (EnginePost[FM_SetModel].at(i), ToCell(ENTINDEX(e)), m));
	RETURN_META(MRES_IGNORED);
}
...
void TraceLine(const float *v1, const float *v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr)
{
	...
	*/
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE(FM_TraceLine, (Engine[FM_TraceLine].at(i), p_v1, p_v2, ToCell(fNoMonsters), ToCell(ENTINDEX(pentToSkip)), ToCell(ptr)));
	RETURN_META(mswi(lastFmRes));
}
...
void TraceLine_post(const float *v1, const float *v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr)
{
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE_POST(FM_TraceLine, (EnginePost[FM_TraceLine].at(i), p_v1, p_v2, ToCell(fNoMonsters), ToCell(ENTINDEX(pentToSkip)), ToCell(ptr)));
	RETURN_META(MRES_IGNORED);
}
...
{
	gfm_tr = ptr;
	FM_ENG_HANDLE(FM_TraceToss, (Engine[FM_TraceToss].at(i), ToCell(ENTINDEX(pent)), ToCell(ENTINDEX(pentToIgnore)), ToCell(ptr)));
	RETURN_META(mswi(lastFmRes));
}
...
{
	gfm_tr = ptr;
	FM_ENG_HANDLE_POST(FM_TraceToss, (EnginePost[FM_TraceToss].at(i), ToCell(ENTINDEX(pent)), ToCell(ENTINDEX(pentToIgnore)), ToCell(ptr)));
	RETURN_META(MRES_IGNORED);
}
...
int TraceMonsterHull(edict_t *pEdict, const float *v1, const float *v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr)
{
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE(FM_TraceMonsterHull, (Engine[FM_TraceMonsterHull].at(i), ToCell(ENTINDEX(pEdict)), p_v1, p_v2, ToCell(fNoMonsters), ToCell(ENTINDEX(pentToSkip)), ToCell(ptr)));
...
int TraceMonsterHull_post(edict_t *pEdict, const float *v1, const float *v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr)
{
	...
	FM_ENG_HANDLE_POST(FM_TraceMonsterHull, (EnginePost[FM_TraceMonsterHull].at(i), ToCell(ENTINDEX(pEdict)), p_v1, p_v2, ToCell(fNoMonsters), ToCell(ENTINDEX(pentToSkip)), ToCell(ptr)));
...
void TraceHull(const float *v1, const float *v2, int fNoMonsters, int hullNumber, edict_t *pentToSkip, TraceResult *ptr)
{
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE(FM_TraceHull, (Engine[FM_TraceHull].at(i), p_v1, p_v2, ToCell(fNoMonsters), ToCell(hullNumber), ToCell(ENTINDEX(pentToSkip)), ToCell(ptr)));
	RETURN_META(mswi(lastFmRes));
}
...
void TraceHull_post(const float *v1, const float *v2, int fNoMonsters, int hullNumber, edict_t *pentToSkip, TraceResult *ptr)
{
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE_POST(FM_TraceHull, (EnginePost[FM_TraceHull].at(i), p_v1, p_v2, ToCell(fNoMonsters), ToCell(hullNumber), ToCell(ENTINDEX(pentToSkip)), ToCell(ptr)));
	RETURN_META(MRES_IGNORED);
}
...
void TraceModel(const float *v1, const float *v2, int hullNumber, edict_t *pent, TraceResult *ptr)
{
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE(FM_TraceModel, (Engine[FM_TraceModel].at(i), p_v1, p_v2, ToCell(hullNumber), ToCell(ENTINDEX(pent)), ToCell(ptr)));
	RETURN_META(mswi(lastFmRes));
}
...
void TraceModel_post(const float *v1, const float *v2, int hullNumber, edict_t *pent, TraceResult *ptr)
{
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE_POST(FM_TraceModel, (EnginePost[FM_TraceModel].at(i), p_v1, p_v2, ToCell(hullNumber), ToCell(ENTINDEX(pent)), ToCell(ptr)));
	RETURN_META(MRES_IGNORED);
}
...
const char *TraceTexture(edict_t *pTextureEntity, const float *v1, const float *v2)
{
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE(FM_TraceTexture, (Engine[FM_TraceTexture].at(i), ToCell(ENTINDEX(pTextureEntity)), p_v1, p_v2));
...
const char *TraceTexture_post(edict_t *pTextureEntity, const float *v1, const float *v2)
{
	...
	FM_ENG_HANDLE_POST(FM_TraceTexture, (EnginePost[FM_TraceTexture].at(i), ToCell(ENTINDEX(pTextureEntity)), p_v1, p_v2));
...
void TraceSphere(const float *v1, const float *v2, int fNoMonsters, float radius, edict_t *pentToSkip, TraceResult *ptr)
{
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE(FM_TraceSphere, (Engine[FM_TraceSphere].at(i), p_v1, p_v2, ToCell(fNoMonsters), radius, ToCell(ENTINDEX(pentToSkip)), ToCell(ptr)));
	RETURN_META(mswi(lastFmRes));
}
...
void TraceSphere_post(const float *v1, const float *v2, int fNoMonsters, float radius, edict_t *pentToSkip, TraceResult *ptr)
{
	...
	PREPARE_VECTOR(v1);
	PREPARE_VECTOR(v2);
	FM_ENG_HANDLE_POST(FM_TraceSphere, (EnginePost[FM_TraceSphere].at(i), p_v1, p_v2, ToCell(fNoMonsters), radius, ToCell(ENTINDEX(pentToSkip)), ToCell(ptr)));
	RETURN_META(MRES_IGNORED);
}
...
{
	FM_ENG_HANDLE(FM_KeyValue, (Engine[FM_KeyValue].at(i), ToCell(ENTINDEX(entity)), ToCell((data))));
	RETURN_META(mswi(lastFmRes));
}

void KeyValue_post(edict_t* entity, KeyValueData* data)
{
	FM_ENG_HANDLE_POST(FM_KeyValue, (EnginePost[FM_KeyValue].at(i), ToCell(ENTINDEX(entity)), ToCell((data))));
	RETURN_META(MRES_IGNORED);
}
...
void AlertMessage(ALERT_TYPE atype, const char *szFmt, ...)
{
	...
	va_end(ap);
	FM_ENG_HANDLE(FM_AlertMessage, (Engine[FM_AlertMessage].at(i), ToCell(atype), buf));
	RETURN_META(mswi(lastFmRes));
}
...
void AlertMessage_post(ALERT_TYPE atype, const char *szFmt, ...)
{
	...
	vsprintf(buf, szFmt, ap);
	va_end(ap);
	FM_ENG_HANDLE_POST(FM_AlertMessage, (EnginePost[FM_AlertMessage].at(i), ToCell(atype), buf));
	RETURN_META(MRES_IGNORED);
}
...
void MoveToOrigin(edict_t *ent, const float *pflGoal, float dist, int iMoveType)
{
	...
	FM_ENG_HANDLE(FM_MoveToOrigin, (Engine[FM_MoveToOrigin].at(i), ToCell(ENTINDEX(ent)), p_pflGoal, dist, ToCell(iMoveType)));
	RETURN_META(mswi(lastFmRes));
}
...
{
	PREPARE_VECTOR(pflGoal);
	FM_ENG_HANDLE_POST(FM_MoveToOrigin, (EnginePost[FM_MoveToOrigin].at(i), ToCell(ENTINDEX(ent)), p_pflGoal, dist, ToCell(iMoveType)));
	RETURN_META(MRES_IGNORED);
}

edict_t *FindEntityByString(edict_t *pEdictStartSearchAfter, const char *pszField, const char *pszValue)
{
	FM_ENG_HANDLE(FM_FindEntityByString, (Engine[FM_FindEntityByString].at(i), ToCell(ENTINDEX(pEdictStartSearchAfter)), pszField, pszValue));
...
edict_t *FindEntityByString_post(edict_t *pEdictStartSearchAfter, const char *pszField, const char *pszValue)
{
	...
	FM_ENG_HANDLE_POST(FM_FindEntityByString, (EnginePost[FM_FindEntityByString].at(i), ToCell(ENTINDEX(pEdictStartSearchAfter)), pszField, pszValue));
...
void GetBonePosition(const edict_t* pEdict, int iBone, float *rgflOrigin, float *rgflAngles)
{
	...
	FM_ENG_HANDLE(FM_GetBonePosition, (Engine[FM_GetBonePosition].at(i), ToCell(ENTINDEX(pEdict)), ToCell(iBone), p_rgflOrigin, p_rgflAngles));
	RETURN_META(mswi(lastFmRes));
}
...
void GetBonePosition_post(const edict_t* pEdict, int iBone, float *rgflOrigin, float *rgflAngles)
{
	PREPARE_VECTOR(rgflOrigin);
	PREPARE_VECTOR(rgflAngles);
	FM_ENG_HANDLE_POST(FM_GetBonePosition, (EnginePost[FM_GetBonePosition].at(i), ToCell(ENTINDEX(pEdict)), ToCell(iBone), p_rgflOrigin, p_rgflAngles));
	RETURN_META(MRES_IGNORED);
}
...
void GetAttachment(const edict_t *pEdict, int iAttachment, float *rgflOrigin, float *rgflAngles)
{
	PREPARE_VECTOR(rgflOrigin);
	PREPARE_VECTOR(rgflAngles);
	FM_ENG_HANDLE(FM_GetAttachment, (Engine[FM_GetAttachment].at(i), ToCell(ENTINDEX(pEdict)), ToCell(iAttachment), p_rgflOrigin, p_rgflAngles));
	RETURN_META(mswi(lastFmRes));
}
...
void GetAttachment_post(const edict_t *pEdict, int iAttachment, float *rgflOrigin, float *rgflAngles)
{
	PREPARE_VECTOR(rgflOrigin);
	PREPARE_VECTOR(rgflAngles);
	FM_ENG_HANDLE_POST(FM_GetAttachment, (EnginePost[FM_GetAttachment].at(i), ToCell(ENTINDEX(pEdict)), ToCell(iAttachment), p_rgflOrigin, p_rgflAngles));
	RETURN_META(MRES_IGNORED);
}
...
void RunPlayerMove(edict_t *fakeclient, const float *viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, byte msec)
{
	...
	FM_ENG_HANDLE(FM_RunPlayerMove, (Engine[FM_RunPlayerMove].at(i), ToCell(ENTINDEX(fakeclient)), p_viewangles, forwardmove, sidemove, upmove, ToCell(buttons), ToCell(impulse), ToCell(msec)));
	RETURN_META(mswi(lastFmRes));
}
...
{
	PREPARE_VECTOR(viewangles);
	FM_ENG_HANDLE_POST(FM_RunPlayerMove, (EnginePost[FM_RunPlayerMove].at(i), ToCell(ENTINDEX(fakeclient)), p_viewangles, forwardmove, sidemove, upmove, ToCell(buttons), ToCell(impulse), ToCell(msec)));
	RETURN_META(MRES_IGNORED);
}
...
void StaticDecal(const float *origin, int decalIndex, int entityIndex, int modelIndex)
{
	...
	FM_ENG_HANDLE(FM_StaticDecal, (Engine[FM_StaticDecal].at(i), p_origin, ToCell(decalIndex), ToCell(entityIndex), ToCell(modelIndex)));
	RETURN_META(mswi(lastFmRes));
}
...
{
	PREPARE_VECTOR(origin);
	FM_ENG_HANDLE_POST(FM_StaticDecal, (EnginePost[FM_StaticDecal].at(i), p_origin, ToCell(decalIndex), ToCell(entityIndex), ToCell(modelIndex)));
	RETURN_META(MRES_IGNORED);
}
...
void BuildSoundMsg(edict_t *entity, int channel, const char *sample, float volume, float attenuation, int fFlags, int pitch, int msg_dest, int msg_type, const float *pOrigin, edict_t *ed)
{
	...
	FM_ENG_HANDLE(FM_BuildSoundMsg, (Engine[FM_BuildSoundMsg].at(i), ToCell(ENTINDEX(entity)), ToCell(channel), sample, volume, attenuation, ToCell(fFlags), ToCell(pitch), ToCell(msg_dest), ToCell(msg_type), p_pOrigin, ToCell(ENTINDEX(ed))));
	RETURN_META(mswi(lastFmRes));
}
...
{
	PREPARE_VECTOR(pOrigin);
	FM_ENG_HANDLE_POST(FM_BuildSoundMsg, (EnginePost[FM_BuildSoundMsg].at(i), ToCell(ENTINDEX(entity)), ToCell(channel), sample, volume, attenuation, ToCell(fFlags), ToCell(pitch), ToCell(msg_dest), ToCell(msg_type), p_pOrigin, ToCell(ENTINDEX(ed))));
	RETURN_META(MRES_IGNORED);
}

int CheckVisibility(const edict_t *entity, unsigned char *pset)
{
	FM_ENG_HANDLE(FM_CheckVisibility, (Engine[FM_CheckVisibility].at(i), ToCell(ENTINDEX(entity)), ToCell(pset)));
	RETURN_META_VALUE(mswi(lastFmRes), (int)mlCellResult);
}
...
{
	g_es_hook = baseline;
	FM_ENG_HANDLE(FM_CreateInstancedBaseline, (Engine[FM_CreateInstancedBaseline].at(i), ToCell(classname), ToCell(baseline)));
	RETURN_META_VALUE(mswi(lastFmRes), (int)mlCellResult);
}
...
int CreateInstancedBaseline_post(int classname, struct entity_state_s *baseline)
{
	g_es_hook = baseline;
	origCellRet = META_RESULT_ORIG_RET(int);
	FM_ENG_HANDLE_POST(FM_CreateInstancedBaseline, (EnginePost[FM_CreateInstancedBaseline].at(i), ToCell(classname), ToCell(baseline)));
	RETURN_META_VALUE(MRES_IGNORED, (int)mlCellResult);
}

char *GetInfoKeyBuffer(edict_t *e)
{
	FM_ENG_HANDLE(FM_GetInfoKeyBuffer, (Engine[FM_GetInfoKeyBuffer].at(i), ToCell(ENTINDEX(e))));
	RETURN_META_VALUE(mswi(lastFmRes), reinterpret_cast<char *>(mlCellResult));
}

char *GetInfoKeyBuffer_post(edict_t *e)
{
	origCellRet = static_cast<cell>(reinterpret_cast<intptr_t>(META_RESULT_ORIG_RET(char *)));
	FM_ENG_HANDLE(FM_GetInfoKeyBuffer, (Engine[FM_GetInfoKeyBuffer].at(i), ToCell(ENTINDEX(e))));
...
{
	FM_ENG_HANDLE(FM_ClientPrintf, (Engine[FM_ClientPrintf].at(i), ToCell(ENTINDEX(pEdict)), ToCell(ptype), szMsg));
	RETURN_META(mswi(lastFmRes));
}
...
{
	g_cd_hook = cd;
	FM_ENG_HANDLE(FM_UpdateClientData, (Engine[FM_UpdateClientData].at(i), ToCell(ENTINDEX(ent)), ToCell(sendweapons), ToCell(cd)));
	RETURN_META(mswi(lastFmRes));
}
...
{
	g_cd_hook = cd;
	FM_ENG_HANDLE_POST(FM_UpdateClientData, (EnginePost[FM_UpdateClientData].at(i), ToCell(ENTINDEX(ent)), ToCell(sendweapons), ToCell(cd)));
	RETURN_META(MRES_IGNORED);
}
...
{
	g_es_hook = state;
	FM_ENG_HANDLE(FM_AddToFullPack, (Engine[FM_AddToFullPack].at(i), ToCell(state), ToCell(e), ToCell(ENTINDEX(ent)), ToCell(ENTINDEX(host)), ToCell(hostflags), ToCell(player), ToCell(pSet)));
	RETURN_META_VALUE(mswi(lastFmRes), (int)mlCellResult);
}
...
int AddToFullPack_post(struct entity_state_s *state, int e, edict_t *ent, edict_t *host, int hostflags, int player, unsigned char *pSet)
{
	g_es_hook = state;
	origCellRet = META_RESULT_ORIG_RET(int);
	FM_ENG_HANDLE_POST(FM_AddToFullPack, (EnginePost[FM_AddToFullPack].at(i), ToCell(state), ToCell(e), ToCell(ENTINDEX(ent)), ToCell(ENTINDEX(host)), ToCell(hostflags), ToCell(player), ToCell(pSet)));
	RETURN_META_VALUE(MRES_IGNORED, (int)mlCellResult);
}
...
void CmdStart(const edict_t *player, const struct usercmd_s *cmd, unsigned int random_seed)
{
	...
	FM_ENG_HANDLE(FM_CmdStart, (Engine[FM_CmdStart].at(i), ToCell(ENTINDEX(player)), ToCell(cmd), ToCell(random_seed)));
	RETURN_META(mswi(lastFmRes));
}
...
{
	g_uc_hook = const_cast<usercmd_t *>(cmd);
	FM_ENG_HANDLE_POST(FM_CmdStart, (EnginePost[FM_CmdStart].at(i), ToCell(ENTINDEX(player)), ToCell(cmd), ToCell(random_seed)));
	RETURN_META(MRES_IGNORED);
}
...
void CreateBaseline(int player, int eindex, struct entity_state_s *baseline, struct edict_s *entity, int playermodelindex, vec3_t player_mins, vec3_t player_maxs)
{
	...
	FM_ENG_HANDLE(FM_CreateBaseline, (Engine[FM_CreateBaseline].at(i), ToCell(player), ToCell(eindex), ToCell(baseline), ToCell(ENTINDEX(entity)), ToCell(playermodelindex), p_player_mins, p_player_maxs));
	RETURN_META(mswi(lastFmRes));
}
...
void CreateBaseline_post(int player, int eindex, struct entity_state_s *baseline, struct edict_s *entity, int playermodelindex, vec3_t player_mins, vec3_t player_maxs)
{
	...
	PREPARE_VECTOR(player_mins);
	PREPARE_VECTOR(player_maxs);
	FM_ENG_HANDLE_POST(FM_CreateBaseline, (EnginePost[FM_CreateBaseline].at(i), ToCell(player), ToCell(eindex), ToCell(baseline), ToCell(ENTINDEX(entity)), ToCell(playermodelindex), p_player_mins, p_player_maxs));
	RETURN_META(MRES_IGNORED);
}
