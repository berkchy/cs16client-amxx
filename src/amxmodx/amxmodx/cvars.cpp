...
static cell AMX_NATIVE_CALL create_cvar(AMX *amx, cell *params)
{
	...
	if (info)
	{
		...
		return static_cast<cell>(reinterpret_cast<intptr_t>(info->var));
	}
...
static cell AMX_NATIVE_CALL get_cvar_pointer(AMX *amx, cell *params)
{
	...
	return static_cast<cell>(reinterpret_cast<intptr_t>(info ? info->var : 0));
}
...
static cell AMX_NATIVE_CALL hook_cvar_change(AMX *amx, cell *params)
{
	...
	return static_cast<cell>(reinterpret_cast<intptr_t>(forward));
}
...
static cell AMX_NATIVE_CALL get_plugins_cvar(AMX *amx, cell *params)
{
	...
	if (info)
	{
		...
		*get_amxaddr(amx, params[6]) = static_cast<cell>(reinterpret_cast<intptr_t>(info->var));
