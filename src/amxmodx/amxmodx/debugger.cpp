...
cell Debugger::_CipAsVa(cell cip)
{
	...
	if (cip >= static_cast<cell>(reinterpret_cast<intptr_t>(code)) && cip < static_cast<cell>(reinterpret_cast<intptr_t>(m_pAmx->base + (int)hdr->dat)))
	{
		return cip - static_cast<cell>(reinterpret_cast<intptr_t>(code));
	} else {
		return static_cast<cell>(reinterpret_cast<intptr_t>(code + cip));
	}
}
...
static cell AMX_NATIVE_CALL dbg_trace_begin(AMX *amx, cell *params)
{
	...
	return static_cast<cell>(reinterpret_cast<intptr_t>(pTrace));
}
...
static cell AMX_NATIVE_CALL dbg_trace_next(AMX *amx, cell *params)
{
	...
	if (pTrace)
		return static_cast<cell>(reinterpret_cast<intptr_t>(pDebugger->GetNextTrace(pTrace)));
