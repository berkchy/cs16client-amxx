...
static cell AMX_NATIVE_CALL param_convert(AMX *amx, cell *params)
{
	...
	* (cell *)(data+(int)amx->frm+(p+2)*sizeof(cell)) -= static_cast<cell>((ptrdiff_t)(data - realdata));
...
{
#if defined JIT && (defined ASM32 || defined AMD64)
	if (!g_Initialized)
...
# endif
	...
	return 1;
#else
	LogError(amx, AMX_ERR_NATIVE, "The JIT compiler is not available in this build of AMX Mod X");
	return 0;
#endif
}
