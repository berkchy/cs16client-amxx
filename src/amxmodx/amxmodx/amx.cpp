...
int AMXAPI amx_Callback(AMX *amx, cell index, cell *result, cell *params)
{
  ...
  *result = f(amx,params);
  ...
  #define RELOC_ABS(base, off)  (*(ucell *)((base)+(int)(off)) += static_cast<ucell>(reinterpret_cast<intptr_t>(base)))
  #define RELOC_VALUE(base, v)  ((v)+static_cast<ucell>(reinterpret_cast<intptr_t>(base)))
#endif
...
int AMXAPI amx_RegisterToAny(AMX *amx, AMX_NATIVE f)
{
  ...
  for (i=0; i<numnatives; i++) {
    if (func->address==0) {
      /* this function is not yet located */
      func->address=static_cast<ucell>(reinterpret_cast<intptr_t>(f));
    } /* if */
    func=(AMX_FUNCSTUB*)((unsigned char*)func+hdr->defsize);
  } /* for */
  if (err==AMX_ERR_NONE)
...
int AMXAPI amx_Reregister(AMX *amx, const AMX_NATIVE_INFO *list, int number)
{
  ...
  for (i=0; i<numnatives; i++) {
    if (func->address!=0) {
      ...
      if (funcptr!=NULL)
      {
        func->address=static_cast<ucell>(reinterpret_cast<intptr_t>(funcptr));
        count++;
      }
    } /* if */
    func=(AMX_FUNCSTUB*)((unsigned char*)func+hdr->defsize);
...
#define ABORT(amx,v)    { (amx)->stk=reset_stk; \
						  (amx)->hea=reset_hea; \
						  (amx)->cip=static_cast<cell>(reinterpret_cast<intptr_t>(cip)); \
						  (amx)->pri=pri; \
						  (amx)->alt=alt; \
						  return v; \
						}
...
static const void * const amx_opcodelist[] = {
  ...
  if ((amx->flags & AMX_FLAG_BROWSE)==AMX_FLAG_BROWSE) {
    ...
    assert(retval!=NULL);
    *retval=static_cast<cell>(reinterpret_cast<intptr_t>(amx_opcodelist));
    return 0;
  } /* if */
